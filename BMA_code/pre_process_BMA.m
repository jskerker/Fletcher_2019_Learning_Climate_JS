...
for dec = 1:6
    ...
    str1 = sprintf('BMA_code/Input/X_%2.0f.csv',1990+20*(dec-1));
    csvwrite(str1,X)
    
    % Calculate lambda priors using std dev of climate data
    SD = [std(X(1,:)), std(X(2,:))]';
    lambda0 = SD.^(2);
    str1 = sprintf('BMA_code/Input/lambda0_%2.0f.csv',1990+20*(dec-1));
...
csvwrite('BMA_code/Input/X0TU.csv',X0_Topts)
csvwrite('BMA_code/Input/X0PU.csv',X0_Popts) 
