...
% Find wherever flex dam is expanded to any threshold (i.e., action >= 4)
for j = 1:4
    indexThresh = zeros(M_T_abs,1);
    for i = 1:M_T_abs
...
legend('boxoff')
text(67, 31.5, 'Expand Dam');
text(78, 27, 'Do not \newlineexpand dam');
sgtitle(['Dam Policies: Large = ', num2str(storage(end)), ...
...
for k = 1%:3
    ...
    if k == 1
        [l1, l3] = legend([b1(2) b1(3) b2], { 'large', 'flex', decade{2:end}, 'never'}, 'FontSize', 10);
...
l = legend(capState(4:end), 'FontSize', 7);
l.Position = l.Position + [-.25 -.5 0 0.1]

t1 = title(['Expansion Decisions (Multiflex), ', 'Non-Optimized Operations: ', num2str(storage(1:end)), ' MCM']);
ylabel('Frequency')
%ylim([0,1200])
xlim([.5,1.5])
set(gca,'XTick',[])
...
for k=1:5
    actCounts(k,:) = histcounts(action(:,k,4), s_C_bins);
    actCounts_test(k,:) = histcounts(action(:,k,4), s_C_bins);
end

for j=2:5
    actCounts(j,2) = actCounts(1,2);
    actCounts(j,3) = actCounts(j-1,3) - sum(actCounts(j,4:end));
    actCounts(j,4:end) = actCounts(j-1,4:end) + actCounts(j,4:end);
end
...
title(['Dam Action Decisions: Small = ', num2str(storage(1)), ...
    ', Large = ', num2str(storage(end))], 'FontSize', 13);
