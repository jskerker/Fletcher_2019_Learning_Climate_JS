...
% Updated by JS- June 2021


clear all
close all
load('Data/Mombasa_TandP-45-85.mat','P0','Pij_85','Tij_85','T0');
Pij = Pij_85;
Tij = Tij_85;
...
%Setup
load('/Users/jenniferskerker/Documents/GradSchool/Research/Project1/Code/Models/Fletcher_2019_Learning_Climate/BMA_code/BMA_results_RCP85_2020-11-14.mat');
...
sgtitle('CMIP-5 Projections over Mwache Basin') 

%% Forecasts based on different transition matrices for precipitation- Added plot (June 2021)

% Plot of RCP8.5 projections using Bayesian model averaging
load('T_Temp_Precip_RCP85A');
%load('T_Temp_Precip_RCP45A');
T_Precip_abs = T_Precip;
...
for t =1:1
    ...
    xlabel('time')
    ylabel('mm/m')
    title('Total P Change Relative to 1990: RCP8.5 Projections')
    %ylim([-18 50])
    set(gca,'Units','normalized')
    if t ==1
    yLabelHandle = get( gca ,'YLabel' );
...
%%
% Subplot 2: Synthetic Scenario 1
% Updating over time
figure;
%mnVal = [-2 0 2];
...
%subplot(1,3,m);
s_P_abs = 59:109;
mn = [1 0 0 0 0];%mnVal(m);
std = 6;
stdChng = 0.8%[1 1.2 0.9 0.8 0.6];%[1 1.2 2 4 1.2];
T_Precip_abs = synthMatrix_stateSpace(mn, std, stdChng, s_P_abs, 0);
T_Precip = T_Precip_abs;
p = randi(numSamp,N-1);
state_ind_P = zeros(1,N);
state_ind_P(1) =  find(P0_abs==s_P_abs);
P_over_time = cell(1,N);
...
for t =1:1
    ...
    xlabel('time')
    ylabel('mm/m')
    title(['Mean = ', num2str(mn), ', StDev = ', num2str(std), ', StDev Chng = ', num2str(stdChng)]);
    %ylim([-18 50])
    set(gca,'Units','normalized')
    if t ==1
    yLabelHandle = get( gca ,'YLabel' );
...
%%
% Subplot 5: Synthetic Scenario 2
% Updating over time
T_Precip_abs = synthMatrix_stateSpace([-2 0 2], 3, 1, 66:97, 0);
p = randi(numSamp,N-1);
state_ind_P = zeros(1,N);
state_ind_P(1) =  find(P0_abs==s_P_abs);
P_over_time = cell(1,N);
...
for t =1:1
    ...
    xlabel('time')
    ylabel('mm/m')
    title('Mean = +/-2, StDev = 3, StDev Chng = 1')
    %ylim([-18 50])
    set(gca,'Units','normalized')
    if t ==1
    yLabelHandle = get( gca ,'YLabel' );
...
% Subplot 2: Synthetic Scenario 3
% Updating over time
T_Precip_abs = synthMatrix_stateSpace([0 -2 -4], 3, .8, 66:97, 0);
p = randi(numSamp,N-1);
state_ind_P = zeros(1,N);
state_ind_P(1) =  find(P0_abs==s_P_abs);
P_over_time = cell(1,N);
...
for t =1:1
    ...
    xlabel('time')
    ylabel('mm/m')
    title('Mean = 0, +2, +4, StDev = 3, StDev Chng = 0.8')
    %ylim([-18 50])
    set(gca,'Units','normalized')
    if t ==1
    yLabelHandle = get( gca ,'YLabel' );
...
% Subplot 4: Synthetic Scenario 4
% Updating over time
T_Precip_abs = synthMatrix_stateSpace([-2 0 2], 6, .8, 66:97, 0);
p = randi(numSamp,N-1);
state_ind_P = zeros(1,N);
state_ind_P(1) =  find(P0_abs==s_P_abs);
P_over_time = cell(1,N);
...
for t =1:1
    ...
    xlabel('time')
    ylabel('mm/m')
    title('Mean = +/-2, StDev = 6, StDev Chng = 0.8')
    %ylim([-18 50])
    set(gca,'Units','normalized')
    if t ==1
    yLabelHandle = get( gca ,'YLabel' );
...
% Subplot 4: Synthetic Scenario 4
% Updating over time
T_Precip_abs = synthMatrix_stateSpace([-2 0 2], 3, .6, 66:97, 0);
p = randi(numSamp,N-1);
state_ind_P = zeros(1,N);
state_ind_P(1) =  find(P0_abs==s_P_abs);
P_over_time = cell(1,N);
...
for t =1:1
    ...
    xlabel('time')
    ylabel('mm/m')
    title('Mean = +/-2, StDev = 3, StDev Chng = 0.6')
    %ylim([-18 50])
    set(gca,'Units','normalized')
    if t ==1
    yLabelHandle = get( gca ,'YLabel' );
