%% DOCUMENTATION:
% This is V3b of multiflex_sdp_climate_StaticFlex.m (date: 6/18/2021)
...
costParam.PercFlex = 0.05; % Jenny used 0.07

% Expansion cost of flexible dam
costParam.PercFlexExp = x(7);
...
storage(3:end) = storage(2) + (1:optParam.numFlex)*optParam.flexIncr; % from (2) to (3)
...
if ~runParam.desalOn
    ...
    for i = 1:optParam.numFlex
        [infra_cost(3), infra_cost(i+3)] = storage2damcost(storage(2), ...
            storage(i+2),costParam.PercFlex); % cost of flexible exp to option X
    end
